...
const DATA_FLOWS: { id: string; source: string; target: string; label: string; animated?: boolean; description: string; phase: FlowPhase; agenticDescription: string; tags: string[] }[] = [
  { id: 'siem_case_management_1', phase: 'ingest', source: 'siem', target: 'case_management', label: 'Alerts', animated: true,
  ...
  { id: 'network_siem_1', phase: 'ingest', source: 'network', target: 'siem', label: 'Flow logs',
    tags: ['Logs', 'Detection'],
  ...
  { id: 'edr_siem_1', phase: 'correlation', source: 'edr', target: 'siem', label: 'Telemetry',
  ...
  { id: 'iam_siem_1', phase: 'correlation', source: 'iam', target: 'siem', label: 'Auth logs',
    tags: ['Logs', 'Detection', 'Correlation'],
  ...
  { id: 'email_case_management_1', phase: 'ingest', source: 'email', target: 'case_management', label: 'Phishing reports',
    tags: ['Alert', 'Logs'],
  ...
  { id: 'threat_intel_case_management_1', phase: 'correlation', source: 'threat_intel', target: 'case_management', label: 'Enrichment', animated: true,
  ...
  { id: 'threat_intel_network_1', phase: 'response', source: 'threat_intel', target: 'network', label: 'IOC feeds',
  ...
  { id: 'threat_intel_edr_1', phase: 'response', source: 'threat_intel', target: 'edr', label: 'IOC feeds',
  ...
  { id: 'case_management_communication_1', phase: 'response', source: 'case_management', target: 'communication', label: 'Notifications', animated: true,
    tags: ['Response', 'Alert'],
  ...
  { id: 'case_management_iam_1', phase: 'response', source: 'case_management', target: 'iam', label: 'Disable accounts',
  ...
  { id: 'case_management_edr_1', phase: 'response', source: 'case_management', target: 'edr', label: 'Containment',
    tags: ['Response', 'Containment'],
  ...
  { id: 'asset_management_case_management_1', phase: 'correlation', source: 'asset_management', target: 'case_management', label: 'Asset context',
  ...
  { id: 'email_threat_intel_1', phase: 'correlation', source: 'email', target: 'threat_intel', label: 'Phishing IOCs',
  ...
  { id: 'cloud_siem_1', phase: 'ingest', source: 'cloud', target: 'siem', label: 'Audit logs', animated: true,
    tags: ['Logs', 'Detection'],
  ...
  { id: 'cloud_iam_1', phase: 'correlation', source: 'cloud', target: 'iam', label: 'Identity events',
  ...
  { id: 'threat_intel_cloud_1', phase: 'correlation', source: 'threat_intel', target: 'cloud', label: 'IOC feeds',
  ...
  { id: 'case_management_cloud_1', phase: 'response', source: 'case_management', target: 'cloud', label: 'Cloud response',
    tags: ['Response', 'Containment'],
  ...
  { id: 'case_management_network_1', phase: 'response', source: 'case_management', target: 'network', label: 'Block rules',
  ...
  { id: 'case_management_email_1', phase: 'response', source: 'case_management', target: 'email', label: 'Quarantine',
    tags: ['Response', 'Containment'],
  ...
  { id: 'edr_case_management_1', phase: 'ingest', source: 'edr', target: 'case_management', label: 'EDR alerts', animated: true,
  ...
  { id: 'asset_management_siem_1', phase: 'ingest', source: 'asset_management', target: 'siem', label: 'Endpoint logs', animated: true,
...
const DEFAULT_HANDLES: Record<string, { sourceHandle: string; targetHandle: string }> = {
  ...
  'cloud_iam_1':                     { sourceHandle: 'bottom-source', targetHandle: 'top-target'   },
  'threat_intel_cloud_1':            { sourceHandle: 'right-source',  targetHandle: 'left-target'  },
  'case_management_cloud_1':         { sourceHandle: 'top-source',    targetHandle: 'top-target'   },
  ...
  'edr_case_management_1':           { sourceHandle: 'bottom-source', targetHandle: 'right-target' },
...
const DEFAULT_WAYPOINTS: Record<string, Array<{ x: number; y: number }>> = {
  ...
  'case_management_cloud_1':         [{ x: 940, y: 230 }, { x: 1080, y: -200 }],
  'iam_siem_1':                      [{ x: 390, y: 170 }, { x: 530.0002648259205, y: 170 }],
  'cloud_siem_1':                    [{ x: 240, y: 36.999984443380555 }, { x: 240, y: 170 }, { x: 379.99998791616144, y: 170 }],
  'network_siem_1':                  [{ x: 960, y: 170 }],
  'case_management_email_1':         [{ x: -90, y: 572.500125916264 }, { x: -90, y: -80 }, { x: 660.0000874212913, y: -80 }],
  'case_management_edr_1':           [{ x: -90, y: 572.4999966397886 }, { x: -90, y: -80 }, { x: 670, y: -80 }],
  'email_threat_intel_1':            [{ x: 1220, y: 320 }, { x: 1044, y: 320 }],
  'edr_case_management_1':           [{ x: 740, y: 720 }, { x: 1380, y: 720 }, { x: 1380, y: 52.499932276342 }],
  'email_case_management_1':         [{ x: 1220, y: 570 }, { x: 924.0000879335148, y: 570 }],
  'asset_management_case_management_1': [{ x: 100, y: 430 }, { x: 730, y: 430 }],
  'threat_intel_network_1':          [{ x: 940, y: 230 }, { x: 1080, y: 50 }, { x: 1080, y: -200 }, { x: 810, y: -200 }, { x: 810, y: 50 }, { x: 846, y: 50 }],
  'edr_siem_1':                      [{ x: 670.0002400192376, y: 170 }, { x: 530, y: 170 }],
              ...
              {allApps.map(({ app, side, catId }) => {
                ...
                return (
                  <Box key={`${side}-${app.name}`} sx={{
                    ...
                    borderRadius: 1.5,
                    border: isDisabled ? '1px solid hsl(var(--border))' : `1px solid hsla(var(${cat?.color || '--primary'}) / 0.2)`,
                    bgcolor: isDisabled ? 'transparent' : `hsla(var(${cat?.color || '--primary'}) / 0.04)`,
                    opacity: isDisabled ? 0.5 : 1,
                    transition: 'all 0.15s ease',
                  }}>
                    <Avatar
                      src={app.image}
                      sx={{ width: 26, height: 26, '& img': { objectFit: 'contain' }, bgcolor: 'hsl(var(--muted))', fontSize: '0.65rem', filter: isDisabled ? 'grayscale(1)' : 'none' }}
                    >
                      {app.name.charAt(0)}
                    </Avatar>
                    <Box sx={{ flex: 1, minWidth: 0 }}>
                      <Typography sx={{ fontSize: '0.78rem', fontWeight: 600, color: isDisabled ? 'hsl(var(--muted-foreground))' : 'hsl(var(--foreground))', lineHeight: 1.2 }}>
                        {app.name}
                      </Typography>
                      <Typography sx={{ fontSize: '0.63rem', color: 'hsl(var(--muted-foreground))' }}>
                    ...
                    <Tooltip title={isDisabled ? 'Enable app' : 'Disable app'} arrow>
                      <Button
                        ...
                        sx={{
                          ...
                          borderRadius: 1,
                          textTransform: 'none',
                          color: isDisabled ? 'hsl(var(--muted-foreground))' : 'hsl(var(--primary))',
                          border: isDisabled ? '1px solid hsl(var(--border))' : '1px solid hsla(var(--primary) / 0.3)',
                          bgcolor: isDisabled ? 'transparent' : 'hsla(var(--primary) / 0.08)',
                          '&:hover': { bgcolor: isDisabled ? 'hsla(var(--muted-foreground) / 0.1)' : 'hsla(var(--primary) / 0.15)' },
                        }}
                      >
