...
import { Shield, Plus, RefreshCw, Search, ArrowRight, Wrench, Globe, LogIn, MonitorCheck } from 'lucide-react';
...
const VulnerabilitiesPage = () => {
    ...
    if (authLoading)
        return null;
    if (!isAuthenticated)
        return <PublicVulnerabilitiesView />;
      ...
      <form onSubmit={handleLookup} className="rounded-lg border border-border bg-card p-5 space-y-3">
        ...
        <div className="flex items-center gap-2">
          <div className="relative flex-1">
            ...
            <Input id="vuln-id" autoFocus placeholder="e.g. CVE-2024-12345 or GHSA-xxxx-xxxx-xxxx" value={query} onChange={(e) => setQuery(e.target.value)} className="pl-8 h-9 text-sm font-mono"/>
          </div>
        ...
        {trimmed && !looksLikeId && (<p className="text-[0.7rem] text-muted-foreground">
...
const AuthenticatedVulnerabilitiesView = () => {
    ...
    const vulnComparisonWorkflow = (workflows || []).find(w => (w.name || '').toLowerCase() === 'vulnerability comparison');
    ...
    const filtered = vulnerabilities.filter(v => {
        if (searchQuery && !v.title.toLowerCase().includes(searchQuery.toLowerCase()) && !v.asset_name?.toLowerCase().includes(searchQuery.toLowerCase()) && !v.cve_id?.toLowerCase().includes(searchQuery.toLowerCase()))
            return false;
    ...
    return (<div className="p-6 max-w-[1400px] mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <IngestionSourcesRow workflowLabel="Ingest Vulnerabilities" category="vulnerabilities" webhookLabel="Ingest Vulnerabilities_webhook" webhookWorkflowName="Vulnerability Ingestion Webhook" titleTooltip="Apps with authentication appear here. Verified apps show in green, unverified in yellow. Toggle them to control which tools automatically pull in vulnerabilities." addSubtitle="Search and authenticate a tool to ingest vulnerabilities from" onSourcesChanged={() => refresh()}/>
      ...
      <div className="grid grid-cols-4 gap-3">
        {['critical', 'high', 'medium', 'low'].map(sev => (<div key={sev} className="rounded-lg border border-border bg-card p-4 cursor-pointer hover:bg-muted/50 transition-colors" onClick={() => setSeverityFilter(severityFilter === sev ? 'all' : sev)}>
      ...
      {vulnerabilities.length > 0 || isLoading ? (<div className="space-y-4">
          {/* Filters */}
          <div className="flex items-center gap-2 flex-wrap justify-end">
            <div className="relative">
              <Search size={14} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-muted-foreground"/>
              <Input placeholder="Search vulns..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-8 h-8 w-[180px] text-sm"/>
            </div>
          ...
          <VulnTable vulnerabilities={filtered} isLoading={isLoading} onRemediate={handleRemediate} emptyIcon={<Shield size={48} className="text-muted-foreground/50 mx-auto mb-4"/>} emptyTitle="No vulnerabilities found" emptyDescription="Connect a vulnerability scanner or sync from a package page to populate this list."/>
        </div>) : (<div className="rounded-lg border border-border bg-card p-12 text-center">
      ...
      <AddVulnerabilityDialog open={addVulnOpen} onOpenChange={setAddVulnOpen} onAdded={() => refresh()}/>
    </div>);
};
const VulnTable = ({ vulnerabilities, isLoading, onRemediate, emptyIcon, emptyTitle, emptyDescription }) => {
    const navigate = useNavigate();
    ...
    if (isLoading) {
        return (<div className="rounded-lg border border-border bg-card p-12 text-center">
    ...
    return (<div className="rounded-lg border border-border bg-card overflow-hidden">
      <Table>
        <TableHeader>
          <TableRow>
        ...
        <TableBody>
          {vulnerabilities.map(vuln => (<TableRow key={vuln.id} className="cursor-pointer hover:bg-muted/30" onClick={(e) => openDetail(vuln.id, e)} onAuxClick={(e) => e.button === 1 && window.open(`/vulnerabilities/${encodeURIComponent(String(vuln.id).split('::')[0])}`, '_blank')}>
              <TableCell>
