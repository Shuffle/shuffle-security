...
import { Laptop, HardDrive, Lock, Package, Zap, Plus, Copy, Check, Activity, ChevronRight, ChevronDown, Radar, FolderOpen, Loader2, CheckCircle2, Send, RefreshCw, ShieldCheck, ShieldX, Cpu, Hash, Clock, Globe, Play, Terminal, Square, Maximize2, Minimize2 } from 'lucide-react';
...
const VulnAssetsPage = () => {
  ...
  return (
    <div className="p-6 max-w-[1400px] mx-auto space-y-6">
      ...
      <Dialog open={!!fullscreenHost} onOpenChange={open => { if (!open) setFullscreenHost(null); }}>
        <DialogContent className="max-w-5xl w-[95vw] h-[85vh] p-0 flex flex-col gap-0 overflow-hidden">
          <DialogHeader className="px-4 py-3 border-b border-border shrink-0">
            <div className="flex items-center gap-3">
              <Terminal size={16} className="text-primary" />
              <div className="flex-1 min-w-0">
                <DialogTitle className="text-sm font-semibold truncate">{fullscreenHost?.hostname}</DialogTitle>
                <DialogDescription className="text-xs text-muted-foreground">
                  {fullscreenHost?.mode === 'full' ? 'Full control (RCE)' : 'Controlled'} · {fullscreenHost?.groupName}
                </DialogDescription>
              </div>
              <Button variant="ghost" size="icon" className="h-7 w-7 shrink-0" onClick={() => setFullscreenHost(null)}>
                <Minimize2 size={14} />
              </Button>
            </div>
          </DialogHeader>
          ...
          {fullscreenHost && (() => {
            const hostHistory = actionHistoryMap.get(fullscreenHost.uuid) || [];
            const actionDebug = hostHistory[hostHistory.length - 1];
            const isRunning = actionDebug && (actionDebug.status === 'sending' || actionDebug.status === 'polling');
            const finishedHistory = hostHistory.filter(e => e.status === 'success' || e.status === 'error');
            const isFull = fullscreenHost.mode === 'full';
              ...
              <>
                ...
                <div className="flex-1 overflow-y-auto min-h-0" ref={(el) => { if (el) el.scrollTop = el.scrollHeight; }}>
                  {finishedHistory.map((entry, i) => {
                    ...
                    return (
                      <div key={i} className={`border-b border-border/50 last:border-b-0 ${isLatest ? 'bg-primary/5 ring-1 ring-inset ring-primary/20' : ''}`}>
                        <div className={`px-4 py-2 flex items-center gap-3 ${isLatest ? 'bg-primary/10' : 'bg-muted/20'}`}>
                        ...
                        {(entry.actionOutput || entry.error) && (
                          <div className="px-4 py-2">
                            {entry.actionOutput && (
                              <pre className="text-xs font-mono text-foreground/80 whitespace-pre-wrap break-words max-h-60 overflow-y-auto">{entry.actionOutput}</pre>
                            )}
                            {entry.error && (
                              <pre className="text-xs font-mono text-destructive whitespace-pre-wrap break-words">{entry.error}</pre>
                            )}
                          </div>
                        )}
                      </div>
                  ...
                  {isRunning && actionDebug && (
                    <div className="border-b border-border/50">
                      <div className="px-4 py-2 flex items-center gap-3 bg-muted/20">
                        <span className="text-xs font-mono text-primary">$</span>
                        <span className="text-sm font-mono font-medium text-foreground flex-1 truncate">{actionDebug.actionName}</span>
                      ...
                      <div className="px-4 py-2 flex items-center justify-between">
                        ...
                        <Button variant="ghost" size="sm" className="h-6 px-3 text-xs text-destructive hover:text-destructive" onClick={() => abortHostAction(fullscreenHost.uuid)}>
                          Stop
                        </Button>
                      </div>
                    </div>
                ...
                <div className="px-4 py-3 flex flex-wrap gap-1.5 border-t border-border/50 shrink-0">
                  {hostActionablePerms.map(perm => (
                    <button
                      key={perm.id}
                      className="px-3 py-1.5 text-xs rounded-md border border-border hover:bg-muted/50 transition-colors text-foreground"
                      onClick={() => executeHostAction(perm.id, perm.name, fullscreenHost.hostname, fullscreenHost.groupName, fullscreenHost.uuid, true)}
                    >
                      {perm.name}
                    </button>
                  ))}
                ...
                <div className="px-4 py-3 border-t border-border shrink-0">
                  <div className="flex gap-2 items-center">
                    ...
                    <Input
                      ...
                      ref={el => { if (el) requestAnimationFrame(() => el.focus()); }}
                      onKeyDown={e => {
                        const history = getCommandHistory(fullscreenHost.uuid);
                        if (e.key === 'Enter' && customAction.trim()) {
                          pushCommandHistory(fullscreenHost.uuid, customAction.trim());
                          setHistoryIndex(-1);
                          executeHostAction(customAction.trim(), customAction.trim(), fullscreenHost.hostname, fullscreenHost.groupName, fullscreenHost.uuid);
                          setCustomAction('');
                        } else if (e.key === 'ArrowUp') {
                          e.preventDefault();
                          setHistoryIndex(prev => {
