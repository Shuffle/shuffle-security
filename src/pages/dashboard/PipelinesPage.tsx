...
- export live=true | sigma "/tmp/sigma_rules" | set uid = uuid() | to "http://example.com/webhook"
...
Use case: ${aiPrompt}`,
  ...
  const availableTemplates = DEFAULT_PIPELINES.map(t => {
    ...
    if (t._dynamicWebhook && !webhookUrl) {
      return { ...t, command: 'export live=true | sigma "/tmp/sigma_rules" | set uid = uuid() | to "<enable webhook on Incidents page>"', hasPlaceholders: true };
    }
    return t;
  });
